 ...
 *
 * Measured on the margin a cohort earns against the margin it has got used to,
 * rather than any fixed figure, for the same reason `prosperity` is: it is
 * free of units and of the price level, and means the same thing in year one
 * as in year forty. A gap rather than a ratio, because margins pass through
 * zero and go negative, and a ratio through zero is meaningless.
 */
export function entryAppetite(world: WorldState, cohort: Cohort): number {
  ...
  const margin = (price - view.wagePerEmployee / productivity) / price;
  const remembered = cohort.pool.marginReference;
  if (remembered === undefined) {
    cohort.pool.marginReference = margin;
    return 1;
  }
  cohort.pool.marginReference = remembered + world.config.firmMarginMemory * (margin - remembered);

  const appetite = 1 + world.config.firmEntryElasticity * (margin - remembered);
  // Bounded so a good year cannot double the firm population and a bad one
